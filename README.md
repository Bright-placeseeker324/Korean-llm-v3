...
> 처음에 기업들이 무료한도를 줄이고 저마저 바이브코딩하는데 한계가 와서 올라마로 돌려보던 중 너무 모델이 무겁고 하다보니 **내가 만들어볼까?**라는 생각이 들었습니다.
> 하지만 중2고 코딩을 그렇게 많이 배운게 아니다보니 모델에 대해 아는건 모델 크기인 B라는 개념밖에 몰랐고 코드 조차 짜지 못했습니다.


> 결국 하던대로 GPT에 들어가 “나 독자적 한국어 LLM 모델 만들래!” 한마디로 시작했습니다.
> 하지만 지피티도 한계가 있다보니 처음에는 코드조각을 받아 차원오류가 안나는지 열심히 돌렸습니다
> 지금은 없지만 처음 v1 이전에 위키 데이터로 학습하는 50M크기의 모델부터 지금까지 키운 것 입니다.
